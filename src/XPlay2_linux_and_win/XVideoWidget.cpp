...
const char* vString = GET_STR(
	...
	varying vec2 textureOut;	//顶点和片元shader共享的变量
	void main(void)
	{
...
//准备yuv数据
//ffmpeg -i v1080.flv -t 10 -s 240x128 -pix_fmt yuv420p out240x128.yuv
...
void XVideoWidget::Init(int width, int height)
{
	...
	//放大过滤，线性差值	GL_NEAREST(效率高，但马赛克严重)，所以使用GL_LINEAR
...
void XVideoWidget::initializeGL()
{
	...
	//fp = fopen("out240x128.yuv", "rb");	//r 读取；b 二进制
	//if (!fp)
	//{
	//	qDebug() << "open out240x128.yuv failed!";
	//}
...
void XVideoWidget::paintGL()
{
	//if (feof(fp))//如果到了文件结尾
	//{
	//	fseek(fp, 0, SEEK_SET);//移到开始位置
	//}
	//fread(datas[0], 1, width*height, fp);
	//fread(datas[1], 1, width*height / 4, fp);
	//fread(datas[2], 1, width*height / 4, fp);
	mux.lock();
	glActiveTexture(GL_TEXTURE0);//激活0层
	...
	glActiveTexture(GL_TEXTURE0 + 1);//激活1层
	...
	glActiveTexture(GL_TEXTURE0 + 2);//激活2层
